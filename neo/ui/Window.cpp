...
const idRegEntry idWindow::RegisterVars[] = {
	...
	{ "alphaFore", idRegister::FLOAT },
	{ "alphaBack", idRegister::FLOAT },
	{ "alphaMat", idRegister::FLOAT },
};
...
void idWindow::Redraw(float x, float y) {
	...
	foreColor.SetAlpha( alphaFore );
	//backColor.SetAlpha( alphaBack );
	matColor.SetAlpha( alphaMat );
...
int idWindow::GetWinVarOffset( idWinVar *wv, drawWin_t* owner) {
	...
	if ( wv == &alphaFore )
	{
		ret = (int)&((idWindow *)0)->alphaFore;
	}

	if ( wv == &alphaBack )
	{
		ret = (int)&((idWindow *)0)->alphaBack;
	}

	if ( wv == &alphaMat )
	{
		ret = (int)&((idWindow *)0)->alphaMat;
	}
...
idWinVar *idWindow::GetWinVarByName(const char *_name, bool fixup, drawWin_t** owner) {
	...
	if ( idStr::Icmp( _name, "alphaFore" ) == 0 )
	{
		retVar = &alphaFore;
	}
	if ( idStr::Icmp( _name, "alphaBack" ) == 0 )
	{
		retVar = &alphaBack;
	}
	if ( idStr::Icmp( _name, "alphaMat" ) == 0 )
	{
		retVar = &alphaMat;
	}
