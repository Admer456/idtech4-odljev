...
bool usercmd_t::operator==( const usercmd_t &rhs ) const {
	return ( buttons == rhs.buttons &&
			...
			upmove == rhs.upmove &&
			leanleft == rhs.leanleft &&
			leanright == rhs.leanright &&
...
void idUsercmdGenLocal::KeyMove( void ) {
	...
	use			= ButtonState( UB_USE );
	leanleft	= ButtonState( UB_LEANLEFT );
	leanright	= ButtonState( UB_LEANRIGHT );
	nightvision = ButtonState( UB_NIGHTVISION );

	cmd.forwardmove = idMath::ClampChar( forward );
	cmd.rightmove	= idMath::ClampChar( side );
	cmd.upmove		= idMath::ClampChar( up );
	cmd.use			= use != 0;
	cmd.leanleft	= leanleft != 0;
	cmd.leanright	= leanright != 0;
	cmd.nightvision = nightvision != 0;
}
...
void idUsercmdGenLocal::MouseMove( void ) {
	...
	if ( sessLocal.player_gameReturn.playerFlags & BIT( 2 ) )
	{
		mx *= 0.5;
		my *= 0.5;
	}
...
/*
==============
idUsercmdGenLocal::KbdButtons
==============
*/
void idUsercmdGenLocal::KbdButtons( void )
{
	cmd.keyboard.oldkeys	= cmd.keyboard.keys;
	cmd.keyboard.oldnumkeys = cmd.keyboard.numkeys;
	cmd.keyboard.keys		= 0;
	cmd.keyboard.numkeys	= 0;

	for ( int i = K_a; i <= K_z; i++ )
		if ( KeyState( i ) )
			cmd.keyboard.keys |= BIT( i - K_a );

	if ( KeyState( K_DOT ) )
		cmd.keyboard.keys |= BIT( KEY_DOT );

	if ( KeyState( K_LPARENTHESIS ) )
		cmd.keyboard.keys |= BIT( KEY_OPENBRACKET );

	if ( KeyState( K_RPARENTHESIS ) )
		cmd.keyboard.keys |= BIT( KEY_CLOSEBRACKET );

	if ( KeyState( K_ENTER ) )
		cmd.keyboard.keys |= BIT( KEY_ENTER );

	if ( KeyState( K_BACKSPACE ) )
		cmd.keyboard.keys |= BIT( KEY_DELETE );

	for ( int i = K_0; i <= K_9; i++ )
		if ( KeyState( i ) )
			cmd.keyboard.numkeys |= BIT( i - K_0 );

	if ( KeyState( K_SPACE ) )
		cmd.keyboard.numkeys |= BIT( NUM_SPACE );

	if ( KeyState( K_PLUS ) )
		cmd.keyboard.numkeys |= BIT( NUM_PLUS );

	if ( KeyState( K_MINUS ) )
		cmd.keyboard.numkeys |= BIT( NUM_MINUS );

	if ( KeyState( K_ASTERISK ) )
		cmd.keyboard.numkeys |= BIT( NUM_MUL );

	if ( KeyState( K_SLASH ) )
		cmd.keyboard.numkeys |= BIT( NUM_DIV );

	if ( KeyState( K_CTRL ) )
		cmd.keyboard.numkeys |= BIT( NUM_LCTRL );

	if ( KeyState( K_DQUOTE ) )
		cmd.keyboard.numkeys |= BIT( NUM_DQUOTE );

	if ( KeyState( K_COMMA ) )
		cmd.keyboard.numkeys |= BIT( NUM_COMMA );

	if ( KeyState( K_SHIFT ) )
		cmd.keyboard.numkeys |= BIT( NUM_SHIFT );
}

// 0-26 = a-z; 27 = comma; 28-29 = brackets; 30 = enter; 31 = backspace/delete 
// 0-9 = 0-9; 10 = space; 11 = +; 12 = -; 13 = *; 14 = /; 15 = lctrl 
