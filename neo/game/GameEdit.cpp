...
void idCursor3D::Present( void ) {
	...
	if ( g_dragShowSelection.GetBool() )
	{
...
==============
*/
void idDragEntity::UpdateDebug( idPlayer *player )
{
...
void idDragEntity::Update( idPlayer *player )
{
	...
	if ( drag )
	{
		if ( player->isUsingAgain )
		{
			if ( player->usercmd.buttons & BUTTON_ATTACK )
			{
				float dragMass = drag->GetPhysics()->GetMass() / 3000;
				dragMass += 1.0;

				drag->ApplyImpulse( player, trace.c.id, trace.c.point, idVec3(150, 500, 200) );
				drag->GetPhysics()->SetLinearVelocity( (idVec3(500,0,0) * viewAxis) / dragMass, trace.c.id );
			}
			...
			// override isUsing and isUsingDragEntity
			// some entities may not perform these two steps
			// in their OnUnuse methods
