...
bool idItem::GiveToPlayer( idPlayer *player ) {
	...
	{ //how the fuck this point is reached?
		return false;
	}
...
bool idItem::Pickup( idPlayer *player )
{
	...
	if ( gameLocal.isClient && gameLocal.mpGame.IsGametypeCoopBased() )
	{ //client-side pickup for coop
		if ( player && si_onePickupPerPlayer.GetBool() && (player->entityNumber != gameLocal.localClientNum) )
		{ //One Pickup per player clientside behaviour
			return false;
		}
	...
	if ( player && si_onePickupPerPlayer.GetBool() && clientPickedItem[ player->entityNumber ] && gameLocal.mpGame.IsGametypeCoopBased() )
	{ //COOP
	...
	if ( !si_onePickupPerPlayer.GetBool() || !gameLocal.mpGame.IsGametypeCoopBased() )
	{ //COOP: enable multiple pickups with si_onePickupPerPlayer enabled
		// clear our contents so the object isn't picked up twice
		GetPhysics()->SetContents( 0 );
	}
...
CLASS_DECLARATION( idItemPowerup, admBurekPowerup )
END_CLASS 

admBurekPowerup::admBurekPowerup()
{
	healthGive = 0;
	temperature = 0;
}

void admBurekPowerup::Save( idSaveGame *savefile ) const
{
	savefile->WriteFloat( healthGive );
	savefile->WriteFloat( temperature );
}

void admBurekPowerup::Restore( idRestoreGame *savefile )
{
	savefile->ReadFloat( healthGive );
	savefile->ReadFloat( temperature );
}

void admBurekPowerup::Spawn()
{
	healthGive = spawnArgs.GetFloat( "health", "20" );
	temperature = spawnArgs.GetFloat( "temperature", "30.0" );
	type = BUREK;
...
bool admBurekPowerup::GiveToPlayer( idPlayer *player )
{
	if ( player->spectating )
	{
		return false;
	...
	player->health += healthGive * ( 1.0 + temperature * 0.01 ); // force health add
	...
	player->GivePowerUp( type, 10 * 1000 );
	return true;
}

void admBurekPowerup::Think( void )
{
	temperature /= 1.0000001;
}
...
bool idPDAItem::GiveToPlayer(idPlayer *player) {
	...
	if ( gameLocal.isClient && gameLocal.mpGame.IsGametypeCoopBased() )
	{ //disable pda pickup in COOP to avoid crash
		return true;
	}
