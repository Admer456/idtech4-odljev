...
#include "engineering/Electro_Base.h"
...
const idEventDef EV_Weapon_ToolUse( "toolUse", NULL, 'd' );
...
CLASS_DECLARATION( idAnimatedEntity, idWeapon )
	...
	EVENT( EV_Weapon_ToolUse,					idWeapon::Event_ToolUse )
...
void idWeapon::Restore( idRestoreGame *savefile ) {
	...
	WEAPON_ZOOMING.LinkTo(		scriptObject, "WEAPON_ZOOMING" );
...
void idWeapon::GetWeaponDef( const char *objectname, int ammoinclip ) {
	...
	isSniper			= weaponDef->dict.GetBool( "sniper", false );
	...
	wallOffsetScale		= weaponDef->dict.GetFloat( "wallOffsetScale", "1.0" );
	wallAngleScale		= weaponDef->dict.GetFloat( "wallAngleScale", "1.0" );
	...
	zoomOffset = weaponDef->dict.GetVector( "zoomOffset", "5 0 0" );
	...
	toolTypeName = weaponDef->dict.GetString( "toolType" );
	if ( toolTypeName[ 0 ] )
	{
		if ( idStr::Ieq( toolTypeName, "Multimeter" ) )
			toolType = TOOL_MULTIMETER;

		else if ( idStr::Ieq( toolTypeName, "Screwdriver" ) )
			toolType = TOOL_SCREWDRIVER;
		
		else if ( idStr::Ieq( toolTypeName, "Cutter" ) )
			toolType = TOOL_CUTTER;
		
		else if ( idStr::Ieq( toolTypeName, "Lockpick" ) )
			toolType = TOOL_LOCKPICK;
		
		else if ( idStr::Ieq( toolTypeName, "Radar" ) )
			toolType = TOOL_RADAR;
		
		else if ( idStr::Ieq( toolTypeName, "Camera" ) )
			toolType = TOOL_CAMERA;
	}
...
void idWeapon::PresentWeapon( bool showViewModel ) {
	...
	static float zoomFactor = 0.0;
	zoomFactor = zoomFactor * 0.92 + (isZooming ? 1.0 : 0.0) * 0.08;

	idVec3 zoomOffsetWorld( 0, 0, 0 );
	idVec3 vecForward, vecRight, vecUp;

	viewWeaponAxis.ToAngles().ToVectors( &vecForward, &vecRight, &vecUp );

	zoomOffsetWorld += zoomOffset.x * vecRight;
	zoomOffsetWorld += zoomOffset.y * vecForward;
	zoomOffsetWorld += zoomOffset.z * vecUp;

	viewWeaponOrigin += (zoomOffsetWorld * zoomFactor) + hideOffset * viewWeaponAxis[ 2 ];

//	common->Printf( "zoomfac %1.3f\n", zoomOffset.x );
	...
	renderEntity.weaponDepthHack = true; // changed to false -Admer
	...
	if ( !gameLocal.isClient ) {
		...
		if ( ent ) {
			...
			if ( ent->fl.takedamage ) {
				...
				float dmgScale = (owner->IsBurekInjected() ? 3.0 : 1.0) * owner->PowerUpModifier( MELEE_DAMAGE );
...
/*
=====================
idWeapon::Event_ToolUse
=====================
*/
void idWeapon::Event_ToolUse( void )
{
	idEntity	*ent;
	trace_t		tr;

	if ( toolType == TOOL_NOT )
	{
		gameLocal.Warning( "\nWeapon's toolType is set to TOOL_NOT, ToolUse is not allowed" );
		return;
	}
	...
	if ( !gameLocal.isClient )
	{
		...
		if ( ent )
		{
			...
			switch ( toolType )
			{
			case TOOL_MULTIMETER:	UseMultimeter( ent );	break;
			case TOOL_CUTTER:		UseCutter( ent );		break;
			case TOOL_SCREWDRIVER:	UseScrewdriver( ent );	break;
			case TOOL_LOCKPICK:		UseLockpick( ent );		break;
			case TOOL_RADAR:		UseRadar( ent );		break;
			case TOOL_CAMERA:		UseCamera( ent );		break;
			}
...
void idWeapon::UseMultimeter( idEntity *ent )
{
	if ( ent->IsType( admElectroBase::Type ) )
	{
...
void idWeapon::UseCutter( idEntity *ent )
{
	if ( ent->IsType( admElectroBase::Type ) )
	{
		admElectroBase *elent = static_cast<admElectroBase*>(ent);
...
void idWeapon::UseScrewdriver( idEntity *ent )
{
	if ( ent->IsType( admElectroBase::Type ) )
	{
		admElectroBase *elent = static_cast<admElectroBase*>(ent);
		elent->OnScrewdriver( this );
	}
}

void idWeapon::UseLockpick( idEntity *ent )
{
/*	if ( ent->IsType( admInteractive::Type ) )
	{
		admInteractive *inent = static_cast<admElectroBase*>(ent);
		inent->OnLockpick();
	}*/
}

void idWeapon::UseRadar( idEntity *ent )
{
/*	UpdateRadar();*/
}

void idWeapon::UseCamera( idEntity *ent )
{
/*	if ( ent->IsType( admInteractive::Type ) )
	{
		admElectroBase *evidence = static_cast<admElectroBase*>(ent);
