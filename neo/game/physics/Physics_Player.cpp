...
const float PM_AIRACCELERATE	= 3.333f;
...
const float	PM_CROUCHFRICTION	= PM_FRICTION * 0.045f;
...
//#define QUAKE_ACCELERATION

#ifndef QUAKE_ACCELERATION
...
void idPhysics_Player::Accelerate( const idVec3 &wishdir, const float wishspeed, const float accel )
{
	...
	idVec3 pushvec, wishvel = wishdir * wishspeed;

//	if ( wishspeed == 0 )
//		return;

	pushvec = wishvel - current.pushVelocity;
	addspeed = pushvec.Normalize();

	accelspeed = accel * frametime * addspeed;
	if ( accelspeed > addspeed )
		accelspeed = addspeed;

	current.velocity += accelspeed * pushvec;
}
	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
		...
		if ( push && trace.c.entityNum != ENTITYNUM_WORLD ) {
			...
			if ( totalMass > 0.0f ) {
				...
				current.velocity *= 1.0f - idMath::ClampFloat( 0.0f, 1000.0f, totalMass - 20.0f ) * ( 1.0f / 750.0f );
				pushed = true;
			}
	...
	// apply ground friction
		...
		if ( !(groundMaterial && groundMaterial->GetSurfaceFlags() & SURF_SLICK) ) {
			...
			if ( !(current.movementFlags & PMF_TIME_KNOCKBACK) ) {
				...
				if ( current.movementFlags & PMF_DUCKED && current.velocity.LengthFast() > 120.0 )
				{
					drop += control * ((1.0 - (0.012 * (current.velocity * gravityNormal) ))) * PM_CROUCHFRICTION * frametime;

					if ( !infiniteSliding )
					{
						drop *= 1.0 + (2.0 * (float)(abs(command.rightmove)) / 127.0);
					}
					...
					common->Printf( "speed %3.2f traction drop %3.2f\n", current.velocity.LengthFast(), drop );
				}
				else
				{
...
void idPhysics_Player::WalkMove( void ) {
	...
	if ( idPhysics_Player::CheckJump() ) {
		...
		if ( waterLevel > WATERLEVEL_FEET ) {
			...
			if ( waterLevel == WATERLEVEL_FEET )
				gameSoundWorld->PlayShaderDirectly( "sound/player/waterjump.ogg", -1 );
...
void idPhysics_Player::LadderMove( void ) {
	...
	float	upscale;
	static float bobbingCycle = 0;

	if ( command.rightmove )
		bobbingCycle += frametime * 3.333;
	else if ( command.forwardmove )
		bobbingCycle += frametime * 6.666;

	if ( bobbingCycle > idMath::PI && command.rightmove )
		bobbingCycle -= idMath::PI;
	else if ( bobbingCycle > idMath::TWO_PI && command.forwardmove )
		bobbingCycle -= idMath::TWO_PI;

	// stick to the ladder
	if ( !command.upmove )
		wishvel = -100.0f * ladderNormal;
	else
		wishvel = 50.0f * ladderNormal;
	...
	if ( command.rightmove ) {
		...
		wishvel += 2.0f * right * sin( bobbingCycle ) * scale * (float) command.rightmove;
	}
	...
	if ( command.forwardmove ) {
		...
		float moveFactor = scale * (float)command.forwardmove;

		wishvel += -0.5f * gravityNormal * fabs( sin( bobbingCycle ) ) * moveFactor;
		wishvel += 0.5f * right * sin( bobbingCycle ) * moveFactor;
	}

	// jump off the ladder
	if ( command.upmove ) {
		wishvel += -3.3f * viewForward;
	}
	...
	if ( command.upmove )
	{
		idVec3 vecForward = idAngles( 0, viewAngles.yaw, 0 ).ToForward();
		current.velocity -= PM_LADDERSPEED * 1.0 * vecForward - PM_LADDERSPEED * 0.333 * -gravityNormal;
	}
...
void idPhysics_Player::MovePlayer( int msec ) {
	...
	if ( waterLevel == WATERLEVEL_WAIST && waterLevelOld < waterLevel && current.velocity.Length() > 300.0 )
	{
		gameSoundWorld->PlayShaderDirectly( "sound/player/watersplash.wav", -1 );
	}
	
	if ( waterLevel == WATERLEVEL_HEAD && waterLevelOld == WATERLEVEL_WAIST && current.velocity.z < 300.0 )
	{
		gameSoundWorld->PlayShaderDirectly( "sound/player/waterenterhead.ogg", SND_CHANNEL_BODY3 );
	}

	if ( waterLevel == WATERLEVEL_NONE && waterLevelOld > waterLevel )
	{
		gameSoundWorld->PlayShaderDirectly( "sound/player/watergetout.wav", SND_CHANNEL_BODY2 );
	}

	if ( waterLevel == WATERLEVEL_WAIST && waterLevelOld == WATERLEVEL_HEAD )
	{
		gameSoundWorld->PlayShaderDirectly( "sound/player/swim1.ogg", SND_CHANNEL_BODY2 );
	}
...
================
*/
void idPhysics_Player::SetInfiniSlide( bool set )
{
	infiniteSliding = set;
}
