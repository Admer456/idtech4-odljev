...
void idPlayerStart::TeleportPlayer( idPlayer *player ) {
	...
	{ //create a new global checkpoint at this position for Coop
		if ( f && ent )
		{
...
===============================================================================
  ...
  Description:
  Vegetation model that bends as the player walks over it.
...
CLASS_DECLARATION( idEntity, admVegetationBendy )
END_CLASS

admVegetationBendy::admVegetationBendy( void )
{
	windDirection = idVec3( 0, 0, 0 );
	originalAngle = idAngles( 0, 0, 0 );
	sway = idAngles( 0, 0, 0 );
	bend = idAngles( 0, 0, 0 );
}

void admVegetationBendy::Save( idSaveGame *savefile ) const
{
	savefile->WriteVec3( windDirection );
	savefile->WriteAngles( originalAngle );
	savefile->WriteAngles( sway );
	savefile->WriteAngles( bend );
}

void admVegetationBendy::Restore( idRestoreGame *savefile )
{
	savefile->ReadVec3( windDirection );
	savefile->ReadAngles( originalAngle );
	savefile->ReadAngles( sway );
	savefile->ReadAngles( bend );
}

void admVegetationBendy::Spawn( void )
{
	windDirection = spawnArgs.GetVector( "windDirection", "0 0 0" );
	sway = spawnArgs.GetAngles( "sway", "0 0 0" );
	bend = spawnArgs.GetAngles( "bend", "0 0 0" );

	model = spawnArgs.GetString( "model" );
	SetModel( model );
...
void admVegetationBendy::ShowEditingDialog( void )
{
	common->InitTool( EDITOR_PARTICLE, &spawnArgs );
}

void admVegetationBendy::Think( void )
{
	idEntity::Think();
}

void admVegetationBendy::Hide( void )
{
	idEntity::Hide();
	GetPhysics()->SetContents( 0 );
}

void admVegetationBendy::Show( void )
{
	idEntity::Show();
	if ( spawnArgs.GetBool( "solid" ) )
	{
...
void admVegetationBendy::Event_Touch( idEntity *other, trace_t *trace )
{
	common->Printf( "admVegetationBendy::Event_Touch" );
	...
	idVec3 playerRight( 0, 0, 0 );
	player->viewAngles.ToVectors( 0, &playerRight, 0 );
	idVec3 entPos = GetPhysics()->GetOrigin();

	idVec3 deltaPos = entPos - playerPos;
	idVec3 deltaPosN = deltaPos.Normalized();
	float distance = deltaPos.Length();

	idAngles deltaPosAngles = deltaPos.ToAngles();
	idVec3 newAxis = idVec3(0,0,0) * idRotation( entPos, playerRight, 15 );
...
===============================================================================
	...
	Description:
	Volume where the player moves like he's in water. No special rendering effects,
	this is merely a functional entity.

===============================================================================
*/

CLASS_DECLARATION( idEntity, admFuncWater )
END_CLASS

admFuncWater::admFuncWater()
{

}

void admFuncWater::Spawn()
{
	BecomeActive( TH_THINK );
	GetPhysics()->SetContents( CONTENTS_WATER );
}

void admFuncWater::Think()
{
	RunPhysics();
}
