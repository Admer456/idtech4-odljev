...
public:
	...
	bool					spawnedByServer;		// When the ent gets spawned by the server; COOP
	bool					clientSideEntity;		// FIXME: I think there's no need for this, but well, for COOP
...
public:
	...
	void					DispatchSpawnThink( void );
	virtual void			SpawnThink( void );		// called only once, to gather extra data *after* all entities have spawned
	...
	bool					hasSpawnThought;

	// usage
	virtual void			OnUse( idPlayer *player );	 // called when the player uses this entity
	virtual void			OnUnuse( idPlayer *player ); // called when the player stops using this entity
	...
	void					FindCustomTargets(  const char* targetname, idList< idEntityPtr<idEntity> > &_targets );
	void					ActivateCustomTargets( idEntity *activator, idList< idEntityPtr<idEntity> >	_targets );
