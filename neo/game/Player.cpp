...
//#include "engineering/WeaponTool.h"
//#include "vehicles/VehicleBase.h"
#include "engineering/Computer_Keyboard.h"
...
CLASS_DECLARATION( idActor, idPlayer )
	...
	EVENT( EV_Footstep,						idPlayer::Event_Footstep )
	EVENT( EV_FootstepLeft,					idPlayer::Event_Footstep )
	EVENT( EV_FootstepRight,				idPlayer::Event_Footstep )
END_CLASS
	...
	if ( !msec ) {
		...
		switch ( powerup ) {
			...
			case ADRENALINE:
				...
				break;
			case BUREK:
				def = gameLocal.FindEntityDef( "powerup_burek", false );
				break;
		}
...
bool idInventory::Give( idPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud ) {
	...
	else if ( idStr::FindText( statname, "givesNightVision" ) == 0 )
	{
		hasNightVision = true;
	}
...
/*
==============
idPlayer::isHoldingKey
==============
*/
bool idPlayer::isHoldingKey( int key )
{
	return usercmd.keyboard.keys & BIT( key );
}

/*
==============
idPlayer::hasStrokedKey
==============
*/
bool idPlayer::hasStrokedKey( int key )
{
	return usercmd.keyboard.keys & BIT( key ) && 
	  !(usercmd.keyboard.oldkeys & BIT( key ));
}
...
idPlayer::idPlayer() {
	...
	memset( &nvgLight,	0, sizeof( nvgLight ) );
	nvgLightHandle			= -1;
...
//	legsModel				= NULL;
	...
	rtcmCameraFront			= NULL;
	rtcmCameraBack			= NULL;
	rtcmCameraRight			= NULL;
	rtcmCameraLeft			= NULL;
	rtcmCameraUp			= NULL;
	rtcmCameraDown			= NULL;
	...
	isUsing					= false;
	isUsingDragEntity		= false;
	isUsingAgain			= false;
	isVehiclePassenger		= false;

	bloodOverlayOpacity		= 2.0;
...
void idPlayer::LinkScriptVariables( void ) {
	...
	AI_SLIDING.LinkTo(			scriptObject, "AI_SLIDING" );
...
void idPlayer::Init( void ) {
	...
	bureksEaten				= 0;
	burekTimer				= 0;
	burekInjection			= false;
	healthCounterTimer		= 0;
	...
	privateCameraView	= NULL;
	rtcmCameraFront		= new idCameraView;
	rtcmCameraBack		= new idCameraView;
	rtcmCameraRight		= new idCameraView;
	rtcmCameraLeft		= new idCameraView;
	rtcmCameraUp		= new idCameraView;
	rtcmCameraDown		= new idCameraView;

	rtcmCameraFront->cinematic = 
	rtcmCameraBack->cinematic = 
	rtcmCameraRight->cinematic = 
	rtcmCameraLeft->cinematic = 
	rtcmCameraUp->cinematic = 
	rtcmCameraDown->cinematic = 0;

	rtcmCameraFront->renderView =
	rtcmCameraBack->renderView =
	rtcmCameraRight->renderView =
	rtcmCameraLeft->renderView =
	rtcmCameraUp->renderView =
	rtcmCameraDown->renderView = NULL;
...
//	SetRtcmCamera( NULL, 0 );
//	SetRtcmCamera( NULL, 1 );
//	SetRtcmCamera( NULL, 2 );
//	SetRtcmCamera( NULL, 3 );
//	SetRtcmCamera( NULL, 4 );
//	SetRtcmCamera( NULL, 5 );
	...
	physicsObj.SetInfiniSlide( g_infiniSlide.GetBool() );
...
//	{
//		legsModel = static_cast<idAnimatedEntity *>(gameLocal.SpawnEntityType( idAnimatedEntity::Type, NULL ));
//
//		idEntity *legsEnt = legsModel.GetEntity();
//
//		const char *modelLegs = spawnArgs.GetString( "model_legs" );
//
//		legsEnt->SetSkin( NULL );
//		if ( modelLegs[ 0 ] )
//		{
//			legsEnt->Show();
//			legsEnt->SetModel( modelLegs );
//
//			if ( legsEnt->GetAnimator()->ModelDef() )
//			{
//				legsEnt->SetSkin( legsEnt->GetAnimator()->ModelDef()->GetDefaultSkin() );
//			}
//
//			legsEnt->GetPhysics()->SetContents( NULL );
//			legsEnt->GetPhysics()->SetClipModel( NULL, 1.0f );
//			legsEnt->GetRenderEntity()->suppressShadowInLightID = entityNumber + 1;
//			legsEnt->Bind( this, true );
//		}
//
//		else
//		{
...
//	}
	...
	hud->SetStateBool( "enableHudAid", g_enableHudAid.GetBool() );
	hud->HandleNamedEvent( "changeHudState" );
}
...
idPlayer::~idPlayer() {
	...
	if ( rtcmCameraFront )
		delete rtcmCameraFront;
	if ( rtcmCameraBack )
		delete rtcmCameraBack;
	if ( rtcmCameraRight )
		delete rtcmCameraRight;
	if ( rtcmCameraLeft )
		delete rtcmCameraLeft;
	if ( rtcmCameraUp )
		delete rtcmCameraUp;
	if ( rtcmCameraDown )
		delete rtcmCameraDown;
...
void idPlayer::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteBool( isUsing );
	savefile->WriteBool( isUsingDragEntity );
	savefile->WriteBool( isVehiclePassenger );
...
void idPlayer::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadBool( isUsing );
	savefile->ReadBool( isUsingDragEntity );
	savefile->ReadBool( isVehiclePassenger );
...
void idPlayer::RestorePersistantInfo( void ) {
	if ( gameLocal.isMultiplayer && (!gameLocal.mpGame.IsGametypeCoopBased() || gameLocal.isClient) )
	{ //if crash then should be server-side only
...
void idPlayer::UpdateHudAmmo( idUserInterface *_hud ) {
	...
	_hud->SetStateFloat( "player_maxammo", ( weapon.GetEntity()->ClipSize() ) );
...
void idPlayer::DrawHUD( idUserInterface *_hud ) {
	...
	// bld2 -= 0.5
	// bld2 = 
	float bloodOverlay2 = ((1.0 - bloodOverlayOpacity) - 0.5) * 1.666;
	if ( bloodOverlay2 < 0.0 )
		bloodOverlay2 = 0.0;

	_hud->SetStateFloat( "bloodoverlay", bloodOverlayOpacity );
	_hud->SetStateFloat( "invbloodoverlay", 1.0 - bloodOverlayOpacity );
	_hud->SetStateFloat( "bloodoverlay2", bloodOverlay2 );
	_hud->HandleNamedEvent( "updateBloodOverlay" );
...
bool idPlayer::GiveItem( idItem *item ) {
	...
	if ( item->spawnArgs.GetBool( "hasNightVision" ) )
	{
		inventory.hasNightVision = true;
	}
...
/*
===============
idPlayer::SetNightVision
===============
*/
void idPlayer::SetNightVision( bool isTrue )
{
	if ( isTrue && inventory.hasNightVision )
	{
		playerFlags |= BIT( PFL_NightVision );
		gameSoundWorld->PlayShaderDirectly( "sound/player/nvg_on.ogg", SND_CHANNEL_PDA );
	}
	else
	{
		playerFlags &= ~(BIT( PFL_NightVision ));

		if ( inventory.hasNightVision )
			gameSoundWorld->PlayShaderDirectly( "sound/player/nvg_off.ogg", SND_CHANNEL_PDA );
	}
}
...
bool idPlayer::GivePowerUp( int powerup, int time ) {
	...
	if ( powerup >= 0 && powerup < MAX_POWERUPS ) {
		...
		switch( powerup ) {
			...
			case BUREK: {
				if ( stamina - 4.0f > 0.0f )
				{
					stamina -= 4.0f; // it takes a bit of force to actually eat a burek
				}

				bureksEaten++;
				thermalUnbalance /= 1.334;
...
void idPlayer::UpdateWeapon( void )
{
	...
	else if ( isUsing )
	{
		...
		if ( isUsingDragEntity )
		{
//			dragEntity.UpdateDebug( this );
			usedDragEntity.Update( this );
		}
	}
	else if ( ActiveGui() )
	{
...
void idPlayer::UpdateFocus( void ) {
	...
	{ //Here to enable Coop players to talk with npcs probably
		allowFocus = false;
	}
	else
	{
...
void idPlayer::BobCycle( const idVec3 &pushVelocity ) {
	...
	// ang = raw 'angle' that we calculate, bang = the angle we blend to
	float			angJump;
	static float	bangJump = 0; // 0 to 180°, thus sin() will be from 0 to 1, then 0 
	float			angRoll;
	static float	bangRoll = 0;

	float			angSwayPitch;
	static float	bangSwayPitch;
	float			angSwayYaw;
	static float	bangSwayYaw;

	float			sliding;
	static float	bSliding = 0;

	static float	bLean = 0;
	
	float			bobJump;
	static float	bbobJump = 0; // b - blended
	static float	hbobJump = 0; // h - holder
	static float	bhbobJump = 0;

	static float	flCycleBobUp = 0;

	static float	flCycleA = 0;
	static float	flCycleB = 0;

	static idVec2	vecUSway = idVec2(0,0);
	...
	if ( physicsObj.HasGroundContacts() ) bobJump = 1.0; else bobJump = 0.0;

	if ( speed > 400.0 ) speed = 400.0;
	else if ( speed < -400.0 ) speed = -400.0;

	sliding = (physicsObj.IsCrouching() && (xyspeed >= 120.0)) ? 1.0 : 0.0;
	bSliding = bSliding * 0.93 + sliding * 0.07;

	// iterate values and limit them
	flCycleBobUp += (float)gameLocal.msec*0.001f;
	flCycleBobUp += speed / 1000;
	flCycleBobUp -= bobJump * 0.008;

	flCycleA += 0.008 * v_viewCycleA.GetFloat();
	flCycleB += 0.008 * v_viewCycleB.GetFloat();

	vecUSway.x += 0.008 * v_viewSwayCycleX.GetFloat();
	vecUSway.y += 0.004 * v_viewSwayCycleY.GetFloat();

	if ( flCycleA > idMath::PI ) // const reminder: PI rad = 180°, 2 PI rad = 360°, PI/2 rad = 90°
		flCycleA = -idMath::PI;

	if ( flCycleB > idMath::PI )
		flCycleB = -idMath::PI;

	if ( flCycleBobUp > idMath::PI )
		flCycleBobUp = -idMath::PI;

	if ( vecUSway.x > idMath::PI ) // range: -180 to +180 (sin is 0, 1, 0)
		vecUSway.x = -idMath::PI;

	if ( vecUSway.y > 2*idMath::PI ) // range: -360 to +360 (sin is 0, 1, 0, -1, 0) 
		vecUSway.y = -2*idMath::PI;

	// Change pitch by jumping and falling
	angJump = velocity.z;

	if ( angJump > 200 ) angJump = 200; else if ( angJump < -200 ) angJump = -200;

	angJump *= 0.45; // 200 units/sec -> 90 degrees
	bangJump = 0.1*angJump + 0.9*bangJump;


	// Change roll by moving left-right (Deathmatch Classic style)
	idVec3 vecRight;
	idVec3 vecForward;
	viewAngles.ToVectors( &vecForward, &vecRight, NULL );
	float flDotRight = vecRight * velocity;
	float flDotForward = vecForward * velocity;
	float speedNormalised = speed;

	if ( speedNormalised > 2000 ) speedNormalised = 2000; else if ( speedNormalised < -2000 ) speedNormalised = -2000;

	speedNormalised /= 2000;

	// calculate angles
	angRoll = flDotRight * 0.25f;
	...
	angRoll	 /= 7.5f;
	bangRoll = 0.1*angRoll + 0.9*bangRoll;
	bbobJump = 0.2*bobJump + 0.8*bbobJump;
	bLean	 = 0.92*bLean   + 0.08*(float)leaningAdjusted;

	if ( velocity.LengthFast() ) 
		flDotForward /= (velocity.LengthFast() * 0.2); 
	else 
		flDotForward = 0;

	vecUSway.x			+= velocity.LengthFast() * 0.0008f;
	vecUSway.y			+= velocity.LengthFast() * 0.0004f;

	angSwayPitch		= sin(vecUSway.x) * velocity.LengthFast() * 0.0125f;
	angSwayYaw			= sin(vecUSway.y) * velocity.LengthFast() * 0.0125f;

	bangSwayPitch		= 0.3*angSwayPitch + 0.7*bangSwayPitch;
	bangSwayYaw			= 0.3*angSwayYaw + 0.7*bangSwayYaw;

	hbobJump			= sin( flCycleBobUp ) * flDotForward * (speedNormalised * speedNormalised * 16.0f) * bbobJump * 2.0f;
	bhbobJump			= hbobJump*0.3 + bhbobJump*0.7;

	// add angles
	viewBobAngles.roll	+= v_rollSide.GetFloat() * bangRoll;
	viewBobAngles.pitch += v_viewJumpAngleFactor.GetFloat() * sin(bangJump * 0.01745327) * 5.0f;
	viewAngles.pitch	+= v_viewSwayPitch.GetFloat() * bangSwayPitch * 0.2f;
	viewAngles.yaw		+= v_viewSwayYaw.GetFloat() * bangSwayYaw * 0.5f;
	viewBob.z			+= v_viewJumpAngleFactor.GetFloat() * bhbobJump;

	viewBobAngles.pitch -= bSliding * 8.0;
	viewBobAngles.roll  -= bSliding * 3.0;
	viewBobAngles.roll	+= v_rollSide.GetFloat() * 4.0 * bSliding * bSliding * bangRoll;

	viewBob				+= vecRight * bLean * 24.0f;
	viewBob.z			-= abs( bLean ) * 4.0f;
	viewAngles.roll		+= bLean * 5.0f;
...
==============
*/
void idPlayer::UseVehicle( idAFEntity_Vehicle *vehicle )
{
	vehicle->Use( this );
}
...
void idPlayer::Use()
{
	...
	start = GetEyePosition();
	end = start + viewAngles.ToForward() * 64;
	gameLocal.clip.TracePoint( trace, start, end, MASK_SHOT_RENDERMODEL, this );
	...
	if ( ent )
	{
		if ( !isUsing )
		{
			...
			{
				Hide();
				UseVehicle( static_cast<idAFEntity_Vehicle*>(ent) );
			}
			
			else if ( usedEntity != ent )
			{
				usedEntity = ent;
				usedEntity->OnUse( this );
			}
		}
			
		else if ( usedEntity/* == ent*/ )
		{
			usedEntity->OnUnuse( this );
			usedEntity = nullptr;
		}
	}
	else if ( isUsing )
...
void idPlayer::PlayFootStepSound()
{
	...
	idPhysics_Player *playerPhys = static_cast<idPhysics_Player*>( GetPhysics() );
	...
	if ( material != NULL )
	{
		if ( playerPhys->GetWaterLevel() != WATERLEVEL_FEET )
		{
			sound = spawnArgs.GetString( va( "snd_footstep_%s", gameLocal.sufaceTypeNames[ material->GetSurfaceType() ] ) );
		}
		else
		{
			sound = spawnArgs.GetString( va( "snd_footstep_water_%s", gameLocal.sufaceTypeNames[ material->GetSurfaceType() ] ) );
		}
	}
	if ( *sound == '\0' )
	{
		...
		{
			sound = spawnArgs.GetString( "snd_footstep_water" );
		}
	}
	if ( *sound != '\0' )
	{
...
void idPlayer::AdjustSpeed( void )
{
	...
	else if ( !physicsObj.OnLadder() && (usercmd.buttons & BUTTON_RUN) && (usercmd.forwardmove || usercmd.rightmove) && (usercmd.upmove >= 0) )
	{
		...
		{	//Here to enable / disabled infinite stamine in COOP
	...
	else
	{
		...
		if ( burekInjection )
			stamina += (rate * PowerUpModifier(BUREK_INJECTION)) * MS2SEC( gameLocal.msec );
...
//	if ( legsModel.GetEntity() )
//	{
//		idAnimator *legsAnimator = legsModel.GetEntity()->GetAnimator();
//
//		legsAnimator->SetJointAxis( hipJoint, JOINTMOD_WORLD, legsAxis );
//
//		legsAnimator->CurrentAnim( ANIMCHANNEL_TORSO )->SetSyncedAnimWeight( 0, downBlend );
//		legsAnimator->CurrentAnim( ANIMCHANNEL_TORSO )->SetSyncedAnimWeight( 1, forwardBlend );
//		legsAnimator->CurrentAnim( ANIMCHANNEL_TORSO )->SetSyncedAnimWeight( 2, upBlend );
//
//		legsAnimator->CurrentAnim( ANIMCHANNEL_LEGS )->SetSyncedAnimWeight( 0, downBlend );
//		legsAnimator->CurrentAnim( ANIMCHANNEL_LEGS )->SetSyncedAnimWeight( 1, forwardBlend );
//		legsAnimator->CurrentAnim( ANIMCHANNEL_LEGS )->SetSyncedAnimWeight( 2, upBlend );
//	}
}
...
void idPlayer::Move( void ) {
	...
	else 
	{
		...
		if ( oldVelocity.Length() > 120.0f && AI_CROUCH )
		{
			AI_SLIDING = true;
		}
		else
...
void idPlayer::Think( void ) 
{
	...
	float bloodOverlayOpacityCurrent = (float)health / spawnArgs.GetFloat( "maxhealth" );
	bloodOverlayOpacity = bloodOverlayOpacity * 0.99 + bloodOverlayOpacityCurrent * 0.01;

//	hud->HandleNamedEvent( "updateBloodOverlay" );

//	common->Printf( "blood: %f\n", hud->GetStateFloat( "bloodoverlay", "0" ) );
	...
	if ( usercmd.nightvision && !oldCmd.nightvision )
		SetNightVision( !(playerFlags & BIT( PFL_NightVision )) );
	...
	if ( isUsingComputer )
	{
		...
		usercmd.rightmove = 0;
		usercmd.upmove = 0;
		usercmd.leanleft = usercmd.leanright = false;
		usercmd.use = false;
		usercmd.impulse = 0;
	}
	...
	if ( inventory.hasNightVision )
	{
		if ( nvgLightHandle == -1 && playerFlags & BIT( PFL_NightVision ) )
		{
			nvgLight.noShadows = true;
			nvgLight.shadowMode = shadowMode_t::NoShadows;
			nvgLight.shaderParms[ SHADERPARM_RED ] = 1.0;
			nvgLight.shaderParms[ SHADERPARM_GREEN ] = 1.0;
			nvgLight.shaderParms[ SHADERPARM_BLUE ] = 1.0;
			nvgLight.origin = this->physicsObj.GetOrigin();
			nvgLight.axis = this->physicsObj.GetAxis();
			nvgLight.pointLight = true;
			nvgLight.lightRadius.x = 2048;
			nvgLight.lightRadius.y = 2048;
			nvgLight.lightRadius.z = 2048;
			nvgLight.shader = NULL;
			nvgLight.suppressLightInViewID = 0;
			nvgLight.allowLightInViewID = 0;
			nvgLight.lightId = LIGHTID_WORLD_MUZZLE_FLASH;
			nvgLightHandle = gameRenderWorld->AddLightDef( &nvgLight );
		}

		if ( nvgLightHandle != -1 )
		{
			if ( playerFlags & BIT( PFL_NightVision ) )
				for ( int i = SHADERPARM_RED; i <= SHADERPARM_BLUE; i++ )
					nvgLight.shaderParms[ i ] = nvgLight.shaderParms[ i ] * 0.99 + 0.23 * 0.01;
			else
				for ( int i = SHADERPARM_RED; i <= SHADERPARM_BLUE; i++ )
					nvgLight.shaderParms[ i ] = 0.0;

			nvgLight.origin = firstPersonViewOrigin + firstPersonViewAxis.ToAngles().ToForward() * 15.0 + idVec3( 0, 0, 8.0 );

			gameRenderWorld->UpdateLightDef( nvgLightHandle, &nvgLight );
		}
	}
	...
	if ( !isUsingComputer )
	{
		...
		if ( usercmd.leanright )
		{
			leaning++;
		}

		PreventLeaningIntoWalls();
		PreventWeaponInWalls();
	}
	...
	if ( burekInjection )
	{
		burekTimer -= MS2SEC( gameLocal.msec );
	...
	{
		if ( health > 100 && healthCounterTimer % 10 == 5 )
			health -= 1;
	}

	healthCounterTimer++;

	if ( health < 75 && healthCounterTimer >= 60 && health > 0 )
	{
		healthCounterTimer = 0;
		health += 1;

		if ( health < 20 )
			healthCounterTimer = 30;

		if ( bureksEaten || burekInjection )
			healthCounterTimer += 20;
	}

//	common->Printf( "health %d stamina %3.2f bureksEaten %d burekTimer %2.2f\n", health, stamina, bureksEaten, burekTimer );
	...
	if ( weapon.GetEntity() ) 
	{
		isZooming =	weapon.GetEntity()->isZooming = usercmd.buttons & BUTTON_ZOOM;
	...
	// calculate the cubemap views
//	CalculateCubemapViews();
	...
	// Obligatory false assignment, used to determine if the player pressed Use() while already using an object
	isUsingAgain = false;

	// Unuse drag entities that are too far away
	if ( isUsingDragEntity )
	{
		if ( usedEntity )
		{
			idVec3 dragEntPos = usedEntity->GetPhysics()->GetOrigin();
			idVec3 playerPos = physicsObj.GetOrigin();
			float distanceDragPlayer = (dragEntPos - playerPos).LengthFast();

			if ( distanceDragPlayer > 96.0 )
			{
				isUsingAgain = true;
				usedEntity->OnUnuse( this );
				usedEntity = nullptr;
			...
			{
				isUsingAgain = true;
				usedEntity->ApplyImpulse( this, 0, usedEntity->GetPhysics()->GetOrigin(), idVec3( 0, 5000, 0 ) );
				usedEntity->OnUnuse( this );
				usedEntity = nullptr;
			}
		}
	...
	// Sniper-specfiic zooming switch
	bool isSniperZoom = false;
	if ( weapon.GetEntity() )
		if ( weapon.GetEntity()->IsSniper() && isZooming )
			isSniperZoom = true;

	if ( isSniperZoom )
		playerFlags |= 1 << PFL_ZoomingSniper;
	else
		playerFlags &= ~(1 << PFL_ZoomingSniper);

	if ( physicsObj.GetWaterLevel() > WATERLEVEL_WAIST )
		playerFlags |= 1 << PFL_Underwater;
	else
		playerFlags &= ~(1 << PFL_Underwater);
}
...
//	if ( hasFirstPersonLegs )
//	{
//		legsModel.GetEntity()->GetAnimator()->ClearAllJoints();
//	}
...
void idPlayer::CalculateViewWeaponPos( idVec3 &origin, idMat3 &axis ) {
	...
	idVec3 vecForward, vecRight, vecUp;
	viewAngles.ToVectors( &vecForward, &vecRight, &vecUp );

	// ang = raw 'angle' that we calculate, bang = the angle we blend to, hbang = another holding angle we blend two
	float			angJump;
	static float	bangJump		= 0; // 0 to 180°, thus sin() will be from 0 to 1, then 0 
	float			angRoll;
	static float	bangRoll		= 0; 
	float			bobJump;
	static float	bbobJump		= 0;
	float			crouch;
	static float	bcrouch			= 0;
	float			deltaRoll;
	static float	bdeltaRoll		= 0;
	float			sliding;
	static float	bSliding		= 0;

	static float	bLean			= 0;
	float			standUp			= 0;
	static float	bstandUp		= 0;
	
	static float	maxHealth		= spawnArgs.GetFloat( "maxhealth" );
	float			healthRatio		= health / maxHealth;

	static float	cycleA			= 0;
	static float	cycleB			= 0;
	...
	float dotForward = velocity.Normalized() * vecForward;
	float dotRight	= velocity.Normalized() * vecRight;
	float dotUp		= velocity.Normalized() * vecUp;

	static float bdotForward = 0;
	static float bdotRight = 0;
	static float bdotUp = 0;
	float		 speed;
	static float bspeed = 0;

	static float bZoom = 0;
	bZoom = bZoom * 0.9 + (isZooming ? 0.0 : 1.0) * 0.1;

	speed = xyspeed;
	if ( speed > 640 ) speed = 640;
	else if ( speed < -640 ) speed = -640;

	speed /= 640;

	if ( speed > 0.64 ) speed = 0.64;
	else if ( speed < -0.64 ) speed = -0.64;
	...
	bLean			= bLean * 0.95		+ (float)leaningAdjusted * 0.05;

	// check if we're on ground and if we're crouching
	if ( physicsObj.HasGroundContacts() ) bobJump = 1.0; 
	else bobJump = 0.0;
	bbobJump = bobJump*0.05 + bbobJump*0.95;

	if ( physicsObj.IsCrouching() ) crouch = 1.0; 
	else crouch = 0.0;
	bcrouch	= crouch*0.05 + bcrouch*0.95;

	if ( !physicsObj.IsCrouching() && bcrouch > 0.3 ) standUp = 1.0;
	else standUp = 0.0;
	bstandUp = standUp*0.035 + bstandUp*0.98;

	// Change pitch by jumping and falling
	angJump = velocity.z;

	if ( angJump > 200 ) angJump = 200; 
	else if ( angJump < -200 ) angJump = -200;

	angJump		*= 0.45; // "converting" from 200 units/sec to 90 degrees
	bangJump	= 0.25*angJump + 0.75*bangJump;

	// compute rolling angle and the delta between the old and new one
	angRoll		= bdotRight;
	bangRoll	= angRoll * 0.15 + bangRoll * 0.85;
	deltaRoll	= angRoll - bangRoll;
	bdeltaRoll	= deltaRoll * 0.1 + bdeltaRoll * 0.9;

	sliding = crouch * (xyspeed > 120.0 ? 1.0 : 0.0);
	bSliding = bSliding * 0.95 + sliding * 0.05;

	// compute view shivering angles
	viewShivering.x = sin( cycleA * 16.0 )	+ 0.25  * cos( cycleB * 8.0 );
	viewShivering.y = cos( cycleA * 12.66 ) + 0.333 * sin( cycleA * 7.0 );
	viewShivering.z = sin( cycleA * 12.66 ) + 0.333 * cos( cycleA * 7.0 );

	viewShivering.x *= sin( cycleA * 20.0 ) * cos( cycleB * 16.0 );
	viewShivering.y *= cos( cycleA * 12.0 ) * sin( cycleB * 26.0 );
...
//	origin		-= v_weaponSwayForward.GetFloat()		* (1.0 - bSliding) * bZoom * bspeed * bbobJump * 1.2f * vecForward * sin( cycleB * 2.0f );
	...
	origin		+= v_crouchPushFr.GetFloat()			/** bspeed*/ * bZoom * bcrouch * vecForward * 10.0f;

	// raise the weapon if we lean, and nudge it a bit sideways: to the left if we lean right and vice-versa
	origin.z	+= bZoom * bLean * bLean * 2.0f;
	origin		+= bZoom * vecRight * bLean * 2.0f;

	// raise the weapon when we stand up
	origin.z	 += bZoom * pow( bstandUp, 2 - (bstandUp * 2) ) * 5.0f;
	angles.pitch += bZoom * pow( bstandUp, 2 - (bstandUp * 2) ) * 5.0f;
	angles.yaw	 -= bZoom * pow( bstandUp, 2 - (bstandUp * 2) ) * 5.0f;

	// shake the weapon if low health, or cold, or spooked
	origin		+= viewShivering * pow((1.0 - healthRatio), 3.0);
	origin		+= viewShivering * spooked;
	origin		+= viewShivering * sqrt(abs( thermalUnbalance ));
	
	// prevent the weapon from going into the wall
	origin		-= vecForward	* weaponWallOffset * 0.5	* wallOffsetScale;
	origin		-= vecUp		* weaponWallOffset * 0.75	* wallOffsetScale;
	angles.pitch -= weaponWallOffset * 2.0 * wallAngleScale;

	// roll the weapon if we lean
	...
	// lower the weapon while sprinting and rotate it left, or right if we run backwards
	angles.pitch += v_weaponSprintPulldown.GetFloat()	* (1.0 - bSliding) * bdotForward * bspeed * 16.0f;
	angles.yaw	-= v_weaponSprintPulldown.GetFloat()	* (1.0 - bSliding) * bdotRight	* (bspeed * bspeed * 12.0f) * 6.0f;

	// push the weapon sideways if we lean, or if we go left-right
	angles.yaw += v_weaponYawPushByRoll.GetFloat()		* bZoom * bdeltaRoll * 8.0;
	angles.yaw += v_weaponYawPush.GetFloat()			* (1.0 - bSliding) * bZoom * bdotForward * (bspeed * bspeed * bspeed * bspeed * 24.0f) * 6.0f;
	...
	angles.roll	 += 8.0 * bSliding * bSliding * bZoom;
	origin.z	 -= 4.0 * bSliding * bZoom;

	cycleA		+= bspeed * 0.0625f;
	cycleB		+= bspeed * 0.0625f;
...
/*
===============
idPlayer::PreventLeaningIntoWalls
===============
*/
void idPlayer::PreventLeaningIntoWalls()
{
	if ( !leaning )
	{
		leaningAdjusted = 0;
	...
	idVec3 start, end;
	idVec3 vecRight;
	viewAngles.ToVectors( NULL, &vecRight, NULL );

	start = GetEyePosition();
	end = start + (leaning * vecRight * 64.0);

	gameLocal.clip.TracePoint( trace, start, end, MASK_OPAQUE, this );

	if ( trace.fraction < 1.0 )
		leaningAdjusted = trace.fraction * leaning;
	else
		leaningAdjusted = 1.0 * leaning;
}

/*
===============
idPlayer::PreventWeaponInWalls
===============
*/
void idPlayer::PreventWeaponInWalls()
{
	trace_t trace;
	idVec3 start, end, vecForward, vecRight, vecUp;

	viewAngles.ToVectors( &vecForward, &vecRight, &vecUp );
	start = GetEyePosition() + viewBob + vecRight * 5.0 - vecUp * 5.0;
	end = start + vecForward * 40.0 + vecUp * weaponWallOffset * 0.6;

	gameLocal.clip.TracePoint( trace, start, end, MASK_OPAQUE, this );

	if ( trace.fraction < 1.0 )
		weaponWallOffset = weaponWallOffset * 0.8 + (40.0 * (1.0 - trace.fraction)) * 0.2;
	else
		weaponWallOffset /= 1.05;

//	gameRenderWorld->DebugLine( idVec4( 1.0, 0.0, 0.0, 0.5 ), start, end );
}
...
void idPlayer::CalculateFirstPersonView( void ) {
	if ( ( pm_modelView.GetInteger() == 1 ) || ( ( pm_modelView.GetInteger() == 2 ) && ( health <= 0 ) ) ) {
		...
		jointHandle_t joint = animator.GetJointHandle( "Head" );
...
/*
==================
idPlayer::CalculateCubemapViews

create renderViews for the current tic
==================
*/
void idPlayer::CalculateCubemapViews( void )
{
	int i;
	float range;
	...
	renderView->y = 0;
	renderView->width = 64;
	renderView->height = 64;
	renderView->viewID = 0;

	rtcmCameraFront->renderView->viewaxis	= axis[ 0 ];
	rtcmCameraBack->renderView->viewaxis	= axis[ 1 ];
	rtcmCameraRight->renderView->viewaxis	= axis[ 2 ];
	rtcmCameraLeft->renderView->viewaxis	= axis[ 3 ];
	rtcmCameraUp->renderView->viewaxis		= axis[ 4 ];
	rtcmCameraDown->renderView->viewaxis	= axis[ 5 ];

	// check if we should be drawing from a camera's POV
	if ( rtcmCameraFront || rtcmCameraBack ||
		 rtcmCameraRight || rtcmCameraLeft ||
		 rtcmCameraUp || rtcmCameraDown )
	{
		rtcmCameraFront->GetViewParms( renderView );
	}
	else
	{
		if ( g_stopTime.GetBool() )
...
void idPlayer::SetInfluenceLevel( int level )
{
	if ( level != influenceActive )
	{
		...
		{
			if ( !gameLocal.mpGame.IsGametypeCoopBased() )
			{ //try to fix crash in coop by Stradex

				for ( idEntity *ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() )
				{ //crash reason in coop
...
void idPlayer::ClientPredictionThink( void ) 
{
	...
	if ( !isLagged ) 
	{
		...
		if ( !noclip && !spectating && (health > 0) && !IsHidden() && gameLocal.isNewFrame && gameLocal.mpGame.IsGametypeCoopBased() )
		{ //Touch triggers COOP
...
/*
==================
idPlayer::Event_Footstep
==================
*/
void idPlayer::Event_Footstep( void )
{
	PlayFootStepSound();
}
...
void idPlayer::Spook( float upTo )
{
	if ( spookedCeiling < upTo )
		spookedCeiling = spookedCeiling * 0.97 + upTo * 0.03;

	common->Printf( "Spook %1.3f", spookedCeiling );
}

void idPlayer::Unspook( float downTo )
{
	if ( spookedCeiling > downTo )
		spookedCeiling = spookedCeiling * 0.97 + ((downTo + spooked + spooked) / 3.0) * 0.03;
}
