...
const char *idAnim::AddFrameCommand( const idDeclModelDef *modelDef, int framenum, idLexer &src, const idDict *def )
{
	...
	else if ( token == "sound_weapon2" )
	{
		if ( !src.ReadTokenOnLine( &token ) )
		{
			return "Unexpected end of line";
		}
		fc.type = FC_SOUND_WEAPON2;
		if ( !token.Cmpn( "snd_", 4 ) )
		{
			fc.string = new idStr( token );
		}
	...
	else if ( token == "sound_weapon3" )
	{
		if ( !src.ReadTokenOnLine( &token ) )
		{
			return "Unexpected end of line";
		}
		fc.type = FC_SOUND_WEAPON3;
		if ( !token.Cmpn( "snd_", 4 ) )
		{
			fc.string = new idStr( token );
		}
	...
	else if ( token == "sound_weapon4" )
	{
		if ( !src.ReadTokenOnLine( &token ) )
		{
			return "Unexpected end of line";
		}
		fc.type = FC_SOUND_WEAPON4;
		if ( !token.Cmpn( "snd_", 4 ) )
		{
			fc.string = new idStr( token );
		}
	...
	while( frame != to ) {
		...
		while( index < end ) {
			...
			switch( command.type ) {
				...
				case FC_SOUND_WEAPON2: {
					if ( !command.soundShader )
					{
						if ( !ent->StartSound( command.string->c_str(), SND_CHANNEL_WEAPON2, 0, false, NULL ) )
						{
							gameLocal.Warning( "Framecommand 'sound_weapon2' on entity '%s', anim '%s', frame %d: Could not find sound '%s'",
											   ent->name.c_str(), FullName(), frame + 1, command.string->c_str() );
						}
					}
					else
					{
						ent->StartSoundShader( command.soundShader, SND_CHANNEL_WEAPON2, 0, false, NULL );
					}
					break;
				}
				case FC_SOUND_WEAPON3: {
					if ( !command.soundShader )
					{
						if ( !ent->StartSound( command.string->c_str(), SND_CHANNEL_WEAPON3, 0, false, NULL ) )
						{
							gameLocal.Warning( "Framecommand 'sound_weapon3' on entity '%s', anim '%s', frame %d: Could not find sound '%s'",
											   ent->name.c_str(), FullName(), frame + 1, command.string->c_str() );
						}
					}
					else
					{
						ent->StartSoundShader( command.soundShader, SND_CHANNEL_WEAPON3, 0, false, NULL );
					}
					break;
				}
				case FC_SOUND_WEAPON4: {
					if ( !command.soundShader )
					{
						if ( !ent->StartSound( command.string->c_str(), SND_CHANNEL_WEAPON4, 0, false, NULL ) )
						{
							gameLocal.Warning( "Framecommand 'sound_weapon4' on entity '%s', anim '%s', frame %d: Could not find sound '%s'",
											   ent->name.c_str(), FullName(), frame + 1, command.string->c_str() );
						}
					}
					else
					{
						ent->StartSoundShader( command.soundShader, SND_CHANNEL_WEAPON4, 0, false, NULL );
					}
					break;
				}
