...
protected:
	...
	// spook the player - 0 means no spook, 1.0 means full spook, 
	// higher values can mean SUPER SPOOK
	void					SpookNearbyPlayers( float upTo, float radius );
	void					UnspookNearbyPlayers( float downTo, float radius );
	...
	void					Event_SpookNearby( float upto, float radius );
	void					Event_UnspookNearby( float downto, float radius );
};
