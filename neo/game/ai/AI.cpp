...
// clear out the enemy when he dies or is hidden
		...
		else if ( gameLocal.mpGame.IsGametypeCoopBased() && thereWasEnemy )
		{ //COOP: probably a player disconnected from server
			ClearEnemy();
		}
...
void idAI::SpookNearbyPlayers( float upTo, float radius )
{
	idPlayer *player;
	int numNearbyEnts = 0;
	idEntity *entityList[ MAX_GENTITIES ];
	idBounds bounds = idBounds( GetPhysics()->GetOrigin() ).Expand( radius );

	numNearbyEnts = gameLocal.clip.EntitiesTouchingBounds( bounds, -1, entityList, MAX_GENTITIES );
	
	for ( int i = 0; i < numNearbyEnts; i++ )
	{
		if ( entityList[ i ]->IsType( idPlayer::Type ) )
		{
			player = static_cast<idPlayer*>(entityList[ i ]);
			player->Spook( upTo );
		}
...
void idAI::UnspookNearbyPlayers( float downTo, float radius )
{
	idPlayer *player;
	int numNearbyEnts = 0;
	idEntity *entityList[ MAX_GENTITIES ];
...
bool  idAI::ClientReceiveEvent( int event, int time, const idBitMsg &msg )
{
	...
	case EVENT_ADD_DAMAGE_EFFECT: {
		...
		if ( (damageDefIndex == -1 || materialIndex == -1 || (AI_DEAD && haveModelDeath)) && gameLocal.mpGame.IsGametypeCoopBased() )
...
void idAI::CSAnimMove( void )
{
	...
	{ //Clients don't know the enemy entity COOP
		TurnToward( turnTowardPos );
		goalPos = oldorigin;
	}
	else if ( GetMovePos( goalPos ) )
