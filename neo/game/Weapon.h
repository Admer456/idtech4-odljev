...
public:
	...
	float					MeasurementSize() const;
	void					Measure( float size );
	bool					IsSniper( void ) { return isSniper; }
	float					GetWallOffset( void ) { return wallOffsetScale; }
	float					GetWallAngleScale( void ) { return wallAngleScale; }
...
private:
	...
	// hiding for not getting into a wall
	float					wallOffsetScale;
	float					wallAngleScale;
	...
	float					measurementSize;	// electrical size we're measuring e.g. voltage from a socket
	...
	idVec3					zoomOffset;			// when we zoom in, we actually aim down the sights thanks to this
	...
	// tool-specific usages
	void					UseMultimeter( idEntity *ent );
	void					UseCutter( idEntity *ent );
	void					UseScrewdriver( idEntity *ent );
	void					UseLockpick( idEntity *ent );
	void					UseRadar( idEntity *ent );
	void					UseCamera( idEntity *ent );
};
