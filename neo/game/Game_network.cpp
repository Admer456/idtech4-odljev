...
idCVar net_clientCoopDebug( "net_clientCoopDebug", "0", CVAR_GAME | CVAR_BOOL | CVAR_NOCHEAT | CVAR_ARCHIVE, "TMP CVar for debugging" );
...
// only implicit materials and sound shaders decls are used
	...
	if ( index >= clientDeclRemap[ localClientNum ][ (int)type ].Num() )
	{
		...
		{ //fixme later, avoid crash in coop
...
void idGameLocal::ServerClientBegin( int clientNum )
{
	...
	if ( mpGame.IsGametypeCoopBased() )
	{
		outMsg.WriteLong( coopIds[ clientNum ] ); //Testing new sync for coop
...
void idGameLocal::SaveEntityNetworkEvent( const idEntity *ent, int eventId, const idBitMsg *msg, bool saveLastOnly )
{
	...
	if ( mpGame.IsGametypeCoopBased() && saveLastOnly )
	{ //used in  coop to avoid overflow of saved events in some entities
		// send all saved events
...
bool idGameLocal::ApplySnapshot( int clientNum, int sequence )
{
	...
	for ( lastSnapshot = NULL, snapshot = clientSnapshots[ clientNum ]; snapshot; snapshot = nextSnapshot )
	{
		...
		if ( snapshot->sequence == sequence )
		{
			...
			{
				if ( mpGame.IsGametypeCoopBased() )
				{ //testing new netcode sync for coop
...
void idGameLocal::ClientShowSnapshot( int clientNum ) const {
	...
	for( ent = snapshotEntities.Next(); ent != NULL; ent = ent->snapshotNode.Next() ) {
		...
		if ( mpGame.IsGametypeCoopBased() )
		{ //added to test new netcode sync coop
...
// SPAWN_PLAYER should be taking care of spawning the entity with the right spawnId
	...
	if ( player->spectating && player->spectator != player->entityNumber && gameLocal.entities[ player->spectator ] && gameLocal.entities[ player->spectator ]->IsType( idPlayer::Type ) )
	{
		...
		weap = static_cast<idPlayer *>(gameLocal.entities[ player->spectator ])->weapon.GetEntity(); // || ADD SEPARATE TOOL THING, 456 ||
...
gameReturn_t idGameLocal::ClientPrediction( int clientNum, const usercmd_t *clientCmds, bool lastPredictFrame ) {
	...
	{ //COOP netcode
		...
		{ //old netcode entities (mover entities mostly)
			for ( ent = snapshotEntities.Next(); ent != NULL; ent = ent->snapshotNode.Next() )
			{
				if ( (ent->entityCoopNumber == player->entityCoopNumber) || !ent->MasterUseOldNetcode() )
				{ //maybe the entity doesn't use oldnetcode but the masters does so do this shit then
					continue;
				}
...
//common->Printf("Ignoring player clientside\n");
	...
	for ( ent = coopSyncEntities.Next(); ent != NULL; ent = ent->coopNode.Next() )
	{
		...
		if ( !ent->fl.hidden && !isSnapshotEntity( ent ) && (ent->snapshotMissingCount[ gameLocal.localClientNum ] >= MAX_MISSING_SNAPSHOTS) )
		{ //probably outside pvs area and not beign sended by Snapshot
			ent->Hide();
	...
	for ( int i = 0; i < MAX_CLIENTS; i++ )
	{
		...
		if ( !coopentities[ i ]->fl.hidden && !isSnapshotEntity( coopentities[ i ] ) && (coopentities[ i ]->snapshotMissingCount[ gameLocal.localClientNum ] >= MAX_MISSING_SNAPSHOTS) )
		{  //probably outside pvs area and not beign sended by Snapshot
			coopentities[ i ]->Hide();
...
int idGameLocal::getFreeEntityNumber( void )
{
	...
	for ( i = CS_ENTITIES_START; i < MAX_GENTITIES; i++ )
	{ //Client-side entities from 3096 to 4095
		ent = entities[ i ];
		if ( !ent )
		{
			return i;
...
// SPAWN_PLAYER should be taking care of spawning the entity with the right spawnId
			...
			if ( mpGame.IsGametypeCoopBased() )
			{ //testing for new netcode sync for coop
				if ( entityDefNumber >= 0 )
				{
					if ( entityDefNumber >= declManager->GetNumDecls( DECL_ENTITYDEF ) )
					{
...
// if the entity is already in the snapshot
		...
		{ //avoid crash related to idEntity::FindTargets coop entities
...
void idGameLocal::ServerWriteSnapshotCoop( int clientNum, int sequence, idBitMsg &msg, byte *clientInPVS, int numPVSClients )
{
	...
	for ( ent = coopSyncEntities.Next(); ent != NULL; ent = ent->coopNode.Next() )
	{
		...
		if ( !ent->IsActive() && !ent->IsMasterActive() && !ent->firstTimeInClientPVS[ clientNum ] && !ent->forceNetworkSync && !ent->inSnapshotQueue[ clientNum ] )
		{ //ignore inactive entities that the player already saw before
			continue;
		}
		// if that entity is not marked for network synchronization
		if ( !ent->fl.coopNetworkSync )
...
void idGameLocal::sendServerOverflowEvents( void )
{
	...
	for ( int i = 0; i < SERVER_EVENTS_QUEUE_SIZE; i++ )
	{
		...
		if ( serverOverflowEvents[ i ].isEventType )
		{ //client joins and read savedqueue events
		...
		{ //from Entity method: ServerSendEvent
