...
/*
================
idEntity::DispatchSpawnThink
================
*/
void idEntity::DispatchSpawnThink( void )
{
	hasSpawnThought = true;
	SpawnThink();
}
...
Called when the player uses an entity.
Entities can have a toggled usage.
Press use -> OnUse
Press use again -> OnUnuse
================
*/
void idEntity::OnUse( idPlayer *player )
{
	player->isUsing = true;
...
However, entities can also have an impulse usage.
Quite simply, call OnUnuse on the end of OnUse. :)
Or don't set isUsing to true, keep it as-is.

Todo: implement holding usage
================
*/
void idEntity::OnUnuse( idPlayer *player )
{
...
bool idEntity::IsMasterActive( void ) const
{
	...
	{ //this shouldn't never happen but weird shit can happen in this Coop tech demo
		return IsActive();
	}
...
===============
*/
void idEntity::FindCustomTargets( const char* targetname, idList< idEntityPtr<idEntity> > &_targets )
{
	int			i;

	// targets can be a list of multiple names
	gameLocal.GetTargets( spawnArgs, _targets, targetname );

	// ensure that we don't target ourselves since that could cause an infinite loop when activating entities
	for ( i = 0; i < _targets.Num(); i++ )
	{
		if ( _targets[ i ].GetEntity() == this )
		{
			gameLocal.Error( "Entity '%s' is targeting itself", name.c_str() );
		}
		//extra for coop: FIXME Search for a clientside workaround for this better
		if ( gameLocal.mpGame.IsGametypeCoopBased() 
			 && _targets[ i ].GetEntity() 
			 && !_targets[ i ].GetEntity()->fl.coopNetworkSync 
			 && (_targets[ i ].GetEntity()->IsType( idAnimated::Type ) 
			 || _targets[ i ].GetEntity()->IsType( idFuncEmitter::Type )) )
		{
			//causing pvs areas crash
			_targets[ i ].GetEntity()->forceNetworkSync = false;
			_targets[ i ].GetEntity()->fl.coopNetworkSync = true;
			gameLocal.RegisterCoopEntity( _targets[ i ].GetEntity() ); //just lol
			_targets[ i ].SetCoopId( gameLocal.GetCoopId( _targets[ i ].GetEntity() ) ); //Dirty dirty hack
			common->Printf( "[COOP] Adding %s to the coopentities array\n", targets[ i ].GetEntity()->GetName() );
		}
	}
}
...
==============================
*/
void idEntity::ActivateCustomTargets( idEntity *activator, idList< idEntityPtr<idEntity> > _targets )
{
	idEntity	*ent;
	int			i, j;
...
bool idEntity::ClientTouchTriggers( void ) const
{
	...
	for ( i = 0; i < numClipModels; i++ )
	{
		...
		if ( !ent->clientSideEntity )
		{ //don't dare to touch snapshot entities
			continue;
		}
