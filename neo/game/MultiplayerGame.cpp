...
void idMultiplayerGame::Run() {
	...
	if ( !gameLocal.coopMapScriptLoad && gameLocal.firstClientToSpawn && IsGametypeCoopBased() && gameLocal.localClientNum < 0 && g_freezeUntilClientJoins.GetBool() )
	{ //first player joined by the dedicated server
	...
	switch( gameState ) {
		...
		case NEXTGAME: {
			if ( nextState == INACTIVE ) {
				...
				if ( gameLocal.gameType == GAME_SURVIVAL || gameLocal.NextMap() )
				{ //in survival automatically do a serverMapRestart after everyone dies
		...
		if ( WantRespawn( p ) || p == spectator )
		{
			...
			else
			{
				...
				else if ( gameLocal.gameType == GAME_SURVIVAL )
				{
					...
					{ //FIXME: SUDDENDEATH IN SURVIVAL?
						if ( playerState[ i ].livesLeft <= 0 )
