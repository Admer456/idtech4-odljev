...
public:
	...
	bool					hasStrokedKey( int key );

	usercmd_t				usercmd;
	usercmd_t				oldusercmd;
	...
	int						playerFlags;		// player flags, used for various effects on the clientside (.exe)
	...
	int						leaning;			// -1 left 0 center 1 right
	float					leaningAdjusted;	// actual leaning, depending on if we go into a wall
	float					weaponWallOffset;	//

	idDragEntity			dragEntity;			// debug drag entity
	idDragEntity			usedDragEntity;		// physics entity that gets dragged when the player uses it
	idEntity				*usedEntity;		// currently used entity

	bool					isUsingAgain;		// player pressed use again after using; gets reset on the end of Think()
	bool					isUsing;			// whether the player is using an entity
	bool					isUsingDragEntity;	// whether the player is using a phys entity
	bool					isVehiclePassenger;

	float					bloodOverlayOpacity;// damage-induced blood overlay
...
public:
	...
	void					CalculateCubemapViews( void );
	void					CalculateFirstPersonView( void );
	void					PreventLeaningIntoWalls( void );
	void					PreventWeaponInWalls( void );
	...
	void					SetNightVision( bool isTrue );
	...
	void					SetRtcmCamera( idCamera *camView, int side );
	...
	bool					IsBurekInjected( void ) { return burekInjection; };
	...
	void					Spook( float upTo );
	void					Unspook( float downTo );
...
private:
	...
	renderLight_t			nvgLight;			// night vision light
	int						nvgLightHandle;

//	idEntityPtr<idAnimatedEntity> legsModel;	// legs that can be seen in 1st person
//	bool					hasFirstPersonLegs;
	...
	int						bureksEaten;
	bool					burekInjection;		// burek superpower switch
	float					burekTimer;
	int						healthCounterTimer;

	idVec3					viewShivering;		// damage-induced shaking, constant when health is low
	float					spooked;			// how scared the player is - from nearby monsters etc.
	float					spookedCeiling;		// the 'target' spooked value 
	float					thermalUnbalance;	// negative values mean cold, positive values mean hot; 
												// absolute 0.0 - normal, 1.0 - cold/hot, 2.0 - very cold/hot, 3.0 - deadly cold/hot
	...
	idCamera *				rtcmCameraFront;
	idCamera *				rtcmCameraBack;
	idCamera *				rtcmCameraRight;
	idCamera *				rtcmCameraLeft;
	idCamera *				rtcmCameraUp;
	idCamera *				rtcmCameraDown;
...
//	void					UseObjects( void );
	...
	void					UseVehicle( idAFEntity_Vehicle *vehicle );
