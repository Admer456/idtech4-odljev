...
idPlayer *idGameLocal::GetCoopPlayer() const
{
	...
	if ( mpGame.IsGametypeCoopBased() )
	{
		if ( localClientNum < 0 || !entities[ localClientNum ] || !entities[ localClientNum ]->IsType( idPlayer::Type ) )
		{
			...
			if ( isServer )
			{ //for coop while using a dedicated server
...
{ //Experimental, maybe can be a bit CPU performance killer with many players
	if ( isClient )
	{
	...
	if ( !isMultiplayer && g_stopTime.GetBool() ) {
		...
		if ( g_timeentities.GetFloat() ) {
			...
			for( ent = activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next() ) {
				...
				if ( !ent->hasSpawnThought )
					ent->DispatchSpawnThink();
				ent->Think();
		...
		if ( !isMultiplayer && player ) {
			...
			ret.bloodOverlay = player->bloodOverlayOpacity;
			ret.playerFlags = player->playerFlags;
...
void idGameLocal::RunDebugInfo( void ) {
	...
	if ( net_clientCoopDebug.GetBool() )
...
void idGameLocal::SetCamera( idCamera *cam ) {
	...
	{ //Clients can't reach this point
		return;
	}
...
void idGameLocal::UpdateServerInfoFlags()
{
	...
	else if ( (idStr::Icmp( serverInfo.GetString( "si_gameType" ), "Coop" ) == 0) )
	{
		gameType = GAME_COOP;
	}
	else if ( (idStr::Icmp( serverInfo.GetString( "si_gameType" ), "Survival" ) == 0) )
	{ //added by Stradex for SURVIVAL COOP
