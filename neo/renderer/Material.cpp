...
void idMaterial::CommonInit() {
	...
	materialFlags = 0;
	realTimeEnvMode = RTEnv_None;
	blendMode = materialBlendMode::BLEND_NONE;
	sort = SS_BAD;
	coverage = MC_BAD;
		...
		if ( numStages && stages )
		{
			...
			for ( i = 0; i < numStages; i++ )
			{
				if ( stages[ i ].lighting == SL_DIFFUSE ||
					 stages[ i ].lighting == SL_DIFFUSE_A ||
					 stages[ i ].lighting == SL_DIFFUSE_B ||
					 stages[ i ].lighting == SL_DIFFUSE_C )
				{
...
int idMaterial::ParseTerm( idLexer &src ) {
	...
	if ( !token.Icmp( "fragmentPrograms" ) ) {
		//TODO(johl): remove this completely? where is it used?
...
void idMaterial::ParseBlend( idLexer &src, shaderStage_t *stage ) {
	...
	if ( !token.Icmp( "bumpmapa" ) )
	{
		stage->lighting = SL_BUMP_A;
		return;
	}
	if ( !token.Icmp( "diffusemapa" ) )
	{
		stage->lighting = SL_DIFFUSE_A;
		return;
	}
	if ( !token.Icmp( "specularmapa" ) )
	{
		stage->lighting = SL_SPECULAR_A;
		return;
	}

	if ( !token.Icmp( "bumpmapb" ) )
	{
		stage->lighting = SL_BUMP_B;
		return;
	}
	if ( !token.Icmp( "diffusemapb" ) )
	{
		stage->lighting = SL_DIFFUSE_B;
		return;
	}
	if ( !token.Icmp( "specularmapb" ) )
	{
		stage->lighting = SL_SPECULAR_B;
		return;
	}

	if ( !token.Icmp( "bumpmapc" ) )
	{
		stage->lighting = SL_BUMP_C;
		return;
	}
	if ( !token.Icmp( "diffusemapc" ) )
	{
		stage->lighting = SL_DIFFUSE_C;
		return;
	}
	if ( !token.Icmp( "specularmapc" ) )
	{
		stage->lighting = SL_SPECULAR_C;
		return;
	}
...
void idMaterial::AddImplicitStages( const textureRepeat_t trpDefault /* = TR_REPEAT  */ ) {
	...
	bool hasDiffuseA = false;
	bool hasSpecularA = false;
	bool hasBumpA = false;

	bool hasDiffuseB = false;
	bool hasSpecularB = false;
	bool hasBumpB = false;

	bool hasDiffuseC = false;
	bool hasSpecularC = false;
	bool hasBumpC = false;
	...
	for ( int i = 0; i < numStages; i++ )
	{
		...
		if ( pd->parseStages[ i ].lighting == SL_BUMP_A )
		{
			hasBumpA = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_DIFFUSE_A )
		{
			hasDiffuseA = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_SPECULAR_A )
		{
			hasSpecularA = true;
		}

		if ( pd->parseStages[ i ].lighting == SL_BUMP_B )
		{
			hasBumpB = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_DIFFUSE_B )
		{
			hasDiffuseB = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_SPECULAR_B )
		{
			hasSpecularB = true;
		}

		if ( pd->parseStages[ i ].lighting == SL_BUMP_C )
		{
			hasBumpC = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_DIFFUSE_C )
		{
			hasDiffuseC = true;
		}
		if ( pd->parseStages[ i ].lighting == SL_SPECULAR_C )
		{
			hasSpecularC = true;
		}
	...
	if ( blendMode == materialBlendMode::BLEND_NONE )
	{
	...
	else if ( blendMode == materialBlendMode::BLEND_2WAY )
	{
		if ( !hasBumpA && !hasDiffuseA && !hasSpecularA &&
			 !hasBumpB && !hasDiffuseB && !hasSpecularB )
		{
			return;
		}
	}
	else if ( blendMode == materialBlendMode::BLEND_3WAY )
	{
		if ( !hasBumpA && !hasDiffuseA && !hasSpecularA &&
			 !hasBumpB && !hasDiffuseB && !hasSpecularB &&
			 !hasBumpC && !hasDiffuseC && !hasSpecularC )
		{
			return;
		}
	}
	...
	else if ( blendMode == materialBlendMode::BLEND_2WAY )
	{
		if ( !hasBumpA )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend bumpmapa\nmap _flat\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "bumpmapa" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}
		...
		if ( !hasBumpB )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend bumpmapb\nmap _flat\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "bumpmapb" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}

		if ( !hasDiffuseA && !hasSpecularA && !hasReflection )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend diffusemapa\nmap _white\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "diffusemapa" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}

		if ( !hasDiffuseB && !hasSpecularB && !hasReflection )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend diffusemapb\nmap _white\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "diffusemapb" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}
	...
	else if ( blendMode == materialBlendMode::BLEND_3WAY )
	{
		...
		if ( !hasBumpC )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend bumpmapc\nmap _flat\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "bumpmapc" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}
		...
		if ( !hasDiffuseC && !hasSpecularC && !hasReflection )
		{
			idStr::snPrintf( buffer, sizeof( buffer ), "blend diffusemapc\nmap _white\n}\n" );
			newSrc.LoadMemory( buffer, strlen( buffer ), "diffusemapc" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			newSrc.FreeSource();
		}
...
void idMaterial::ParseMaterial( idLexer &src ) {
	...
	while ( 1 ) {
		...
		// realTimeEnv <mode>
		// SSR -> use Screen Space Reflections
		// RTCM -> use Real-Time Cube Map
		else if ( !token.Icmp( "realTimeEnv" ) )
		{
			src.ReadTokenOnLine( &token );

			if ( !token.Icmp( "SSR" ) )
				realTimeEnvMode = RTEnv_SSR;
			else if ( !token.Icmp( "RTCM" ) )
				realTimeEnvMode = RTEnv_CM;
		...
		else if ( !token.Icmp( "materialBlendMode" ) )
		{
			src.ReadTokenOnLine( &token );
			if ( !token.Icmp( "None" ) )
			{
			...
			else if ( !token.Icmp( "VertexColorGrayscale" ) )
			{
				blendMode = materialBlendMode::BLEND_2WAY;
			}
			else if ( !token.Icmp( "VertexColorRGB" ) )
			{
				blendMode = materialBlendMode::BLEND_3WAY;
			}
			continue;
		}
		...
		// diffusemapa for stage shortcut
		else if ( !token.Icmp( "diffusemapa" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
		...
		// diffusemapb for stage shortcut
		else if ( !token.Icmp( "diffusemapb" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
		...
		// diffusemapc for stage shortcut
		else if ( !token.Icmp( "diffusemapc" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
		...
		// specularmapa for stage shortcut
		else if ( !token.Icmp( "specularmapa" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
			R_ParsePastImageProgram( src, str );
			idStr::snPrintf( buffer, sizeof( buffer ), "blend specularmapa\nmap %s\n}\n", str );
			newSrc.LoadMemory( buffer, strlen( buffer ), "specularmapa" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			continue;
		}
		// specularmapb for stage shortcut
		else if ( !token.Icmp( "specularmapb" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
			R_ParsePastImageProgram( src, str );
			idStr::snPrintf( buffer, sizeof( buffer ), "blend specularmapb\nmap %s\n}\n", str );
			newSrc.LoadMemory( buffer, strlen( buffer ), "specularmapb" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			continue;
		}
		// specularmapc for stage shortcut
		else if ( !token.Icmp( "specularmapc" ) )
		{
			idLexer newSrc;
			char str[ MAX_IMAGE_NAME ];
			char buffer[ 1024 ];
			R_ParsePastImageProgram( src, str );
			idStr::snPrintf( buffer, sizeof( buffer ), "blend specularmapc\nmap %s\n}\n", str );
			newSrc.LoadMemory( buffer, strlen( buffer ), "specularmapc" );
			newSrc.SetFlags( LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
			ParseStage( newSrc, trpDefault );
			continue;
		}
...
bool idMaterial::Parse( const char *text, const int textLength ) {
	...
	for ( i = 0 ; i < numStages ; i++ ) {
		...
		pStage->blendMode = this->blendMode;
