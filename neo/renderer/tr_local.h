...
{
	BLEND_NONE,
	BLEND_2WAY, // grayscale vertex colours
	BLEND_3WAY, // Red -> material A, Green -> material B, Blue -> material C
	BLEND_4WAY, // blend 3way + Alpha -> material D
};
...
typedef struct {
	...
	idImage *			bumpImage;		// RGB - normals, A - heightmap
	idImage *			diffuseImage;	// RGB - colours, A - opacity
	idImage *			specularImage;	// R - roughness, G - metallic, B - AO, A - unused!

	idImage *			blendADiffuseImage;
	idImage *			blendBDiffuseImage;
	idImage *			blendCDiffuseImage;

	idImage *			blendASpecularImage;
	idImage *			blendBSpecularImage;
	idImage *			blendCSpecularImage;

	idImage *			blendABumpImage;
	idImage *			blendBBumpImage;
	idImage *			blendCBumpImage;

//	TO-DO: Implement
//	These are optional textures which won't affect the rendering of a surface in any way, if not included in the material definition
//	idImage *			pbrImage;		// so we'll support both PBR and 2004 Phong
//	idImage *			detailImage;	// detail map
//	idImage *			detailBumpImage;// detail normal map
//	idImage *			parallaxImage;	// parallax offset bump mapping
//	idImage *			dirtImage;		// overlay of "dirt"
 
	// Real-time cubemaps
	idImage *			rtcmFrontImage;
	idImage *			rtcmBackImage;
	idImage *			rtcmRightImage;
	idImage *			rtcmLeftImage;
	idImage *			rtcmUpImage;
	idImage *			rtcmDownImage;
	idImage *			rtcmImage;

	bool				hasSSR;			// screen space reflection
	bool				hasRTCM;		// realtime cubemap

	materialBlendMode	blendMode;		// in what way should multiple materials blend
...
public:
	...
	virtual void			CaptureRenderToImage( const char *imageName, renderView_t *ref ) override;
...
extern idCVar r_skipGlsl;			// bypasses all glsl stages in materials
