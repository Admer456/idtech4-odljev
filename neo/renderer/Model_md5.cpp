...
====================
*/
void idMD5Mesh::UpdateSurface( const struct renderEntity_t *ent, const idJointMat *entJoints, modelSurface_t *surf ) const {
	int i, base;
	srfTriangles_t *tri;
	...
	// jmarshall - No renderEntity support
...
idRenderModel *idRenderModelMD5::InstantiateDynamicModel( const struct renderEntity_t *ent, const struct viewDef_s *view, idRenderModel *cachedModel ) {
	...
	/*if ( !ent->joints ) {
		common->Printf( "Warning: idRenderModelMD5::InstantiateDynamicModel: NULL joints on renderEntity for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	} else if ( ent->numJoints != joints.Num() ) {
		common->Printf( "Error: idRenderModelMD5::InstantiateDynamicModel: renderEntity has different number of joints than model for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	}*/

	// jmarshall - md5mesh without animation
	...
	// jmarshall - md5mesh without animation
	if ( r_showSkel.GetInteger() && ent != nullptr )
	{
		if ( ( view != NULL ) && ( !r_skipSuppress.GetBool() || !ent->suppressSurfaceInViewID || ( ent->suppressSurfaceInViewID != view->renderView.viewID ) ) )
		{
	// only draw the skeleton
	...
	for( mesh = meshes.Ptr(), i = 0; i < meshes.Num(); i++, mesh++ ) {
		...
		if ( ent == nullptr )
		{
			mesh->UpdateSurface( nullptr, &poseMat3[ 0 ], surf );
		}
		else
		{
			if ( ent->HasValidJoints() )
			{
				mesh->UpdateSurface( ent, ent->joints, surf );
			}
			else
			{
				mesh->UpdateSurface( ent, &poseMat3[ 0 ], surf );
			}
		}
		// jmarshall end
